...
int execute_command(char *command) {
    ...
    if (pid == -1) {
        ...
        // Handle the shell logical operators
        if (strcmp(args[arg_count - 1], "&&") == 0 && WIFEXITED(status) && WEXITSTATUS(status) == 0
        )
       	{
            // The command succeeded, continue execution
            return 0;
        } else if (strcmp(args[arg_count - 1], "||") == 0 && (WIFEXITED(status) && WEXITSTATUS(status) != 0)) {
            // The command failed, continue execution
            return 0;
        } else {
            // The command did not meet the condition, stop execution
            return 1;
        }
    }
}
